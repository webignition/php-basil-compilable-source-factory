...
class TryCatchBlockFactory
{
    ...
    public function create(
        ...
        ClassNameCollection $catchClasses,
        BodyInterface $catchBody,
    ): TryCatchBlock {
        $objectTypeDeclarations = [];
        foreach ($catchClasses as $catchClass) {
            $objectTypeDeclarations[] = new ObjectTypeDeclaration(new ClassName($catchClass->getClassName()));
        }
        ...
        return new TryCatchBlock(
            ...
            new CatchBlock(
                new CatchExpression(
                    new ObjectTypeDeclarationCollection($objectTypeDeclarations)
                ),
                $catchBody
            ),
        );
